...
別途記録されたライブ確認では、合成Wiseメールによる新規マーチャント経路、LINEカテゴリ選択からSheets更新までの経路、英語形式BOG SMS 4件のバックフィル、同一Messages GUIDの重複停止が観測されています。一方、BOGのジョージア語形式・新規マーチャント・解析失敗経路、Gmailの自然なスケジュール起動には未検証項目があります。
