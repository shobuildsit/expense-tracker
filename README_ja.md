...
- ソースごとに適した解析方式 — 自由形式のWiseメールはOpenAI Structured Outputs（マーチャント・GEL・JPY）、固定フォーマットのBOG SMSは決定的なテキスト解析（LLM不使用）。どちらも必須項目を抽出できない場合は推測値を記録せず、アラート送信のうえ行を追加しない設計
...
- SMSの例としてはBank of Georgiaの1銀行の固定フォーマットのみを対象とし、そのフォーマットに合わせて決定的に解析しています。他銀行フォーマットへの対応はパーサーの行・ラベル一致ロジックの調整で可能ですが、自動化はしていません
